...
for (let element of Array.from(document.getElementsByTagName('textarea'))) {
  element.addEventListener('input', adjustHeight);
}
ccfoUnitElement.removeEventListener('input',adjustHeight);
...
memoElement.onchange = (e) => {
  namePreviewElement.innerText = e.currentTarget.value ? e.currentTarget.value.split('\n')[0] : '探索者名';
};
...
colorTextElement.oninput = (e) => {
  if (/^#?[0-9a-fA-F]{6}$/.test(e.currentTarget.value)) {
    let color = (e.currentTarget.value.charAt(0)=='#'?'':'#') + e.currentTarget.value;
...
// テーブル：✔切り替え
Array.from(paramsTable.children).forEach( row => {row.onclick = e => {
  toggleSecretCheckbox(e);
  let i = chatList.findIndex(element=>element.name.startsWith(e.currentTarget.children[1].innerText));
...
Array.from(statsTable.children).forEach( row => {row.onclick = e => {
  toggleSecretCheckbox(e);
  if (e.currentTarget.children[1].innerText=='SAN') {
    let i = chatList.findIndex(element=>String(element.value).startsWith('1d100<={SAN}'));
    if (i>-1)  chatTable.children[i].firstElementChild.firstElementChild.checked = e.currentTarget.firstElementChild.firstElementChild.checked;
  }
}});
...
function adjustHeight (e) { e.currentTarget.rows = e.currentTarget.value.split('\n').length }
...
/**
 * 指定のテーブルに行を追加する関数
 * @param {HTMLTableElement} table 行を追加するテーブルのTbodyエレメント
 ...
 */
function addRow(table, thCol=-1, draggable=false, editable=false) {
  const cols = table.parentElement.getElementsByTagName('thead')[0].getElementsByTagName('tr')[0].childElementCount;
...
function importUnit (e) {
  ...
  // face
  if (unitdata.data.faces) {
    diffElement.value = unitdata.data.faces.filter(e => e.label).map(e => e.label).join('\n');
    diffElement.rows  = unitdata.data.faces.filter(e => e.label).length;
  }
  ...
  if (unitdata.data.params) {
    ...
    let extraParams = unitdata.data.params.filter(e => !/STR|CON|POW|DEX|APP|SIZ|INT|EDU|DB/.test(e.label));
  ...
  // commands
  {
    let arr = unitdata.data.commands.match(/(\d+).*アイディ?ア|アイディ?ア.*@(\d+)/);
    if(arr) statsElement.value += `\nアイデア  ${arr[1]||arr[2]}`;

    arr = unitdata.data.commands.match(/(\d+).*幸運|幸運.*@(\d+)/);
    if(arr) statsElement.value += `\n幸運  ${arr[1]||arr[2]}`;
    
    arr = unitdata.data.commands.match(/(\d+).*知識|知識.*@(\d+)/);
    if(arr) statsElement.value += `\n知識  ${arr[1]||arr[2]}`;
  }
  statsElement.rows = statsElement.value.split('\n').length+1;

  skillsElement.value = unitdata.data.commands.replace(/^.*<=\{.*\}.*$/mg,'').replace(/^.*(?:アイディ?ア|幸運|知識).*$/mg,'').replace(/ /g,'_').trim();
  skillsElement.rows = skillsElement.value.split('\n').length;
...
function updateValueTables () {
  ...
  for (let key of ['STR','CON','POW','DEX','APP','SIZ','INT','EDU'] ) {
    const arr = text.match(new RegExp(`${key}\\W*(\\d+)`));
    props[key.toLowerCase()] = arr ? parseInt(arr[1]) : null;
  }
  // DB
  {
    const arr = text.match(/(?:DB|ダメージ・?ボーナス)\W*([-D\d]+)/i);
    props.db = arr ? arr[1].toLowerCase() : null;
    if( !props.db && props.str && props.siz ) {
      let sum = (props.str + props.siz) / (coc6th ? 1 : 5);
  ...
  // HP・MP・SAN
  {
    let arr = text.match(/(?:HP|耐久力?)\D*(\d+)/i);
    props.hp = arr ? parseInt(arr[1]) : props.con&&props.siz ? (coc6th ? Math.ceil((props.con+props.siz)/2) : Math.floor((props.con+props.siz)/10)) : null;
    
    arr = text.match(/(?:MP|マジック・?ポイント)\D*(\d+)/i);
    props.mp = arr ? parseInt(arr[1]) : props.pow ? props.pow/(coc6th?1:5) : null;
    
    arr = text.match(/(?:SAN値?|正気度)\D*(\d+)/i);
    props.san = arr ? parseInt(arr[1]) : props.pow ? props.pow*(coc6th?5:1) : null;
  }
  ...
  // アイデア・幸運・知識
  {
    let arr = text.match(/(?:ID[AE]|アイディ?ア)\D*(\d+)/i);
    props.ide  = arr ? parseInt(arr[1]) : props.int ? props.int*(coc6th?5:1) : null;
 
    arr = text.match(/(?:LUCK|幸運)\D*(\d+)/i);
    props.luck = arr ? parseInt(arr[1]) : props.pow&&coc6th ? props.pow*5 : null;
    
    arr = text.match(/(?:KNOW|知識)\D*(\d+)/i);
    props.know = arr ? parseInt(arr[1]) : props.edu ? props.edu*(coc6th?5:1) : null;
  }
...
function remakeskillTable () {
  ...
  const pattern = new RegExp(`${b}(?:[-+*/]${b})*`,'i');

  for (let base of baseArr) {
    let dic, times;
    ...
    if (/^[\w ]*choice/i.test(base)) {  // choice
      dic = {type:'choice', name:'', value:base};

    } else if (/^[\w ]*(?:CBR|RES)/i.test(base)) {  // 対抗ロール
      dic = {type:'elseRoll', name:'', value:base};

    } else if (/(?:1d100|CCB?)<=\(?\d/i.test(base)) { // CCB<=70 skill
      let arr = base.match(/<=([-+*/()\d]+) *(.*)/);
      dic = {type:'roll', name:arr[2], value:arr[1]};

    } else if (/(?:1d100|CCB?).*@\d+$/i.test(base)) { // CCB skill @70
      let arr = base.match(/(?:1d100|CCB?) *(.*) *@(\d+)$/i)
      dic = {type:'roll', name:arr[1], value:arr[2]};

    } else if (/\dD\d/i.test(base)) { // 1d3, 1d3+db
      const arr = base.match(pattern);
      const name = base.replace(arr[0],'');
      const value = arr[0].replace(/\/1$/i,'').replace(/\{?db\}?/gi,'{DB}');
      dic = {type:'dice', name:name, value:value};

    } else if (/チョイス/.test(base)) { // チョイスA,B,C
      let arr = base.replace(/^.*チョイス/,'').match(/\((.*)\)/);
      let choices = arr ? arr[1] : base.replace(/^.*チョイス/,'');
      let val = choices.split(/[,、 ]/).filter(e => e.length>0).join(',');
      let name = arr ? base.match(/\)(.*)/)[1] : '';
      dic = {type:'choice', name:name, value:`choice(${val})`};

    } else {  // skill 70
      let arr = base.match(/(.*?)(\(?\d[-+*/\d()]*)\D*$/i);
      if (!arr) {
...
function makeChatList(skillList) {
  ...
  if(statsTable.children[2].lastElementChild.innerText) {
    let dic = {type:'elseRoll', name:'', value:'1d100<={SAN} 【正気度ロール】'};
...
/**
 * チャパレテーブルからチャパレテキストを作成する関数
 * @param {Event} event
 * @returns {text} チャパレテキスト
 */
function getChatpalette() {
  return Array.from(chatTable.children).filter((e) => !e.firstElementChild.firstElementChild.checked).map((e) => e.lastElementChild.innerText).join('\n');
}

function exportUnit(event) {
  // const coc6th = Boolean(parseInt(systemElements.filter(e => e.checked)[0].value));
  ...
  // params
  Array.from(paramsTable.children).filter((e)=>!e.firstElementChild.firstElementChild.checked).forEach((row)=>{
  ...
  // stats
  Array.from(statsTable.children).filter((e)=>!e.firstElementChild.firstElementChild.checked).forEach((row)=>{
  ...
  if (diffElement.value) {
    for (let row of diffElement.value.split('\n')) {
      unit.data.faces.push({iconUrl:null, label:row});
    };
  }
